...
class StageWindow : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        //ボス用のコース定義出力。
        myString6 = EditorGUILayout.TextField("ボス用のコース定義", myString6);
        //ボス用のコース定義の情報のボタンが押されtureが返って来る。
        if (GUILayout.Button("ボス用のコース定義の情報を出力"))
        {
            //空白の時の例外処理
            if (myString6 == "")
            {
            ...
            {
                //ボス用のコース定義の情報の出力の処理
                OutputBossCourceDef.ShowWindow(myString6);
                Debug.Log("ボス用のコース定義の情報が無事出力できました。");
            }
        }
