...
void CShadowSamplingRender_I::Initialize(){
	...
	//	(*graphicsDevice()).CreateVertexDeclaration(declElement, &Decl);
	//	m_VertexDecl.reset(Decl);
	//	m_IsFirst = false;
	//}
}
...
void CShadowSamplingRender_I::CreateMatrixBuffer(unsigned int MaxNum){
	...
	//	(*graphicsDevice()).CreateVertexBuffer(sizeof(D3DXMATRIX)* MaxNum, 0, 0, D3DPOOL_MANAGED, &pVB, 0);
	//	m_WorldMatrixBuffer.reset(pVB);
	//	m_MaxNum = MaxNum;
	//}
}
...
void CShadowSamplingRender_I::AnimationDraw(ANIMATION::D3DXMESHCONTAINER_DERIVED* pMeshContainer, ANIMATION::D3DXFRAME_DERIVED* pFrame){
...
void CShadowSamplingRender_I::NonAnimationDraw(ANIMATION::D3DXFRAME_DERIVED* pFrame){
	...
	//	(*graphicsDevice()).SetVertexDeclaration(m_VertexDecl.get());

	//	(*graphicsDevice()).SetStreamSource(0, vb, 0, stride);							//頂点バッファをストリーム0番目に設定
	//	(*graphicsDevice()).SetStreamSource(1, m_WorldMatrixBuffer.get(), 0, sizeof(D3DXMATRIX));	//ワールド行列用のバッファをストリーム1番目に設定。
	//	//ワールド行列を頂点バッファにコピー。
