...
void CSceneResult::Initialize(){
	...
	SINSTANCE(CObjectManager)->AddObject(&m_rBack, _T("ResultBack"), OBJECT::PRIORTY::OBJECT2D, false);
	SINSTANCE(CObjectManager)->AddObject(&m_rSelect[0], _T("Continue"), OBJECT::PRIORTY::OBJECT2D,false);
	//m_rSelect[1].SetFileName(_T("image/result_EXIT.png"));
	//SINSTANCE(CObjectManager)->AddObject(&m_rSelect[1], _T("GoTitle"), false);
	SINSTANCE(CObjectManager)->AddObject(&m_rCursor, _T("ResultCursor"), OBJECT::PRIORTY::OBJECT2D,false);
