...
// ピクセルシェーダ
float4 PS_Main(VS_OUTPUT In, uniform Flags flags){
	...
	//if (flags.Fresnel){
	//	// 視線の反射ベクトル算出。
	//	float3 vReflect = reflect(g_EyeDir, normal);
	//		float4 work1 = texCUBE(cubeTexSampler, vReflect);
	//		// 視線の屈折ベクトル算出。
	//		float3 vRefract = refract(g_EyeDir, normal, g_Refractive);
	...
	//	// 求めた反射率でブレンディング。
	//	light = (work1 * fresnel) + (work2 * (1.0f - fresnel));
	//}
